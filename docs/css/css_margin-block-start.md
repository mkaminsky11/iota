...
### Syntax
    ...
    margin-block-start: 1em       /* A length relative to the text size */
    margin-block-start: 5%        /* A margin relative to the nearest block container's width */
...
The `margin-block-start` property takes the same values as the [`margin-left`][5] property.
...
The definition of 'margin-block-start' in that specification.][19]
Editor's Draft
Initial definition
...
[19]: http://dev.w3.org/csswg/css-logical-props/#propdef-margin-block-start
