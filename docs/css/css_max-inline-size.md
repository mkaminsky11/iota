...
The `max-inline-size` [CSS][1] property defines the horizontal or vertical maximal size of an element's block depending on its writing mode. It corresponds to the [`max-width`][2] or the [`max-height`][3] property depending on the value defined for [`writing-mode`][4]. If the writing mode is vertically oriented, the value of `max-inline-size` relates to the maximal height of the element, otherwise it relates to the maximal width of the element. It relates to [`max-block-size`][5], which defines the other dimension of the element.
...
### Syntax
    ...
    max-inline-size: 300px           /* <length> values */
    max-inline-size: 25em
    
    max-inline-size: 75%             /* <percentage> values */
    
    max-inline-size: none            /* Keyword values */
...
The `max-inline-size` property takes the same values as the [`max-width`][2] and [`max-height`][3] properties.
...
The definition of 'max-inline-size' in that specification.][14]
Editor's Draft
Initial definition
...
[5]: https://developer.mozilla.org/en/docs/Web/CSS/max-block-size "The max-block-size CSS property defines the horizontal or vertical maximal size of an element's block depending on it's writing mode. It corresponds to the max-width or the max-height property depending on the value defined for writing-mode."
...
[14]: http://dev.w3.org/csswg/css-logical-props/#propdef-max-inline-size
